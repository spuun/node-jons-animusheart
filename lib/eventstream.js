...
class EventStream {
  ...
  _connectEventStream() {
    ...
    socket.on('message', async data => {
      ...
      if (!func.functionProperties) {
        return this.logger.warn(`[WS] function has no functionProperties functionUID=${event.functionUID}`)
      }
      const cls = func.functionProperties.data.metadata.clazz.split('.').pop()
      this._eventSubscribers.forEach(callback => {
        callback.call(this, {
          device: device,
          data: new (functionData[cls])(event.value)
        })
      })
    })
